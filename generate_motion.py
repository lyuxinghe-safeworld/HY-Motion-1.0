...
Produces NPZ files identical to local_infer.py output, consumable by
hymotion_isaaclab's convert_npz.py. Also renders a skeleton animation MP4
...
Usage:
    ...
    # Generate motion + skeleton video
    HY_MOTION_LLM_4BIT=1 python generate_motion.py \
        --prompt "A person walks forward" \
        --output-dir output/generated/

    # Then convert to .motion for Isaac Lab tracking (in env_isaaclab):
...
def save_npz(model_output, output_dir, base_filename):
    ...
    Uses construct_smpl_data_dict to ensure identical output.
    """
...
def main():
    ...
    print()
    print("Next step — convert to .motion for Isaac Lab tracking:")
    print(f"  cd ~/code/hymotion_isaaclab")
    print(f"  PYTHONPATH=\"$HOME/code/ProtoMotions:$HOME/code/ProtoMotions/data/scripts:$PYTHONPATH\" \\")
    print(f"      python scripts/convert_npz.py --npz-file {npz_paths[0]} --output-dir output/")
    print("=" * 60)
